...
export function HotelElevator3D({
  ...
  const elevator = useHotelStore(
    (state) => state.elevator
  )

  const callElevator = useHotelStore(
    (state) => state.callElevator
  )

  const moveElevator = useHotelStore(
    (state) => state.moveElevator
  )
  ...
  useFrame((_, delta) => {
    ...
    const targetY =
      elevator.targetFloor * FLOOR_HEIGHT + 0.15

    const diff =
      targetY - cabin.position.y

    if (Math.abs(diff) > 0.035) {
      cabin.position.y +=
        diff * Math.min(1, delta * 2.2)

      moveElevator(
        cabin.position.y / FLOOR_HEIGHT
      )
    } else {
