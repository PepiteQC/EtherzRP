...
export interface FluidConfig extends Omit<FluidSimulationOptions, 'profile'> {
  ...
  /**
   * Active le champ de teinture (dye) pour les traînées colorées
   */
  enableDye: boolean
  /**
   * Advection BFECC (plus nette, ~5× le coût)
   */
  bfec: boolean
}
...
export class EtherFluidSimulation {
  ...
  async init(renderer: WebGLRenderer, config?: Partial<FluidConfig>): Promise<boolean> {
    try {
      ...
      this._fluid = new FluidSimulation(renderer as any, {
        ...
        bfecc: this._config.bfec,
        pressureIterations: profileParams.pressureIterations,
      })
      // enableDye est une propriété d'instance (pas une option du constructeur)
