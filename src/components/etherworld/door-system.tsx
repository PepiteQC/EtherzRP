...
// 3D room compatibility exports used by EtherWorldRoom.
...
export function SecurityDoor({ position = [0, 0, 0], rotation = [0, 0, 0] }: RoomObjectProps) {
  return (
    <group position={position} rotation={rotation}>
      <mesh position={[0, 1, 0]} castShadow receiveShadow>
        <boxGeometry args={[0.9, 2, 0.08]} />
        <meshStandardMaterial color="#172036" metalness={0.25} roughness={0.65} />
      </mesh>
      <mesh position={[0.32, 1, 0.06]}>
        <sphereGeometry args={[0.04, 12, 12]} />
        <meshStandardMaterial color="#f5c542" metalness={0.8} roughness={0.2} />
      </mesh>
    </group>
  )
}

export function BathroomFixtures({ position = [0, 0, 0] }: RoomObjectProps) {
  return (
...
export function WindowWithCurtains({ position = [0, 0, 0], rotation = [0, 0, 0], size = [2, 1.4] }: RoomObjectProps) {
  ...
  return (
    <group position={position} rotation={rotation}>
      <mesh>
        ...
        <meshStandardMaterial color="#7dd3fc" transparent opacity={0.35} metalness={0.1} roughness={0.05} />
      </mesh>
      <mesh position={[-width / 2 - 0.08, 0, 0.03]}>
        <boxGeometry args={[0.12, height + 0.25, 0.05]} />
